...
impl GgufValue {
    ...
    pub fn as_u32_array(&self) -> Option<Vec<u32>> {
        match self {
            Self::Array(values) => values.iter().map(|value| value.as_u32()).collect(),
            _ => None,
        }
    }
}
...
impl GgufFile {
    ...
    pub fn get_u32_array_mean(&self, key: &str) -> Option<u32> {
        let values = self
            .metadata
            .get(key)
            .and_then(|value| value.as_u32_array())
            .or_else(|| {
                let arch = self.get_string("general.architecture")?;
                self.metadata
                    .get(&format!("{arch}.{key}"))
    ...
    pub fn get_u32_array_sum(&self, key: &str) -> Option<u32> {
        let values = self
            .metadata
            .get(key)
            .and_then(|value| value.as_u32_array())
        ...
        Some(values.iter().map(|value| *value as u32).sum())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_parse_u32_array_metadata_helpers() {
        let mut buf = Vec::new();
        write_gguf_header(&mut buf, 3, 0, 2);
        write_kv_string(&mut buf, "general.architecture", "gemma4");
        write_kv_u32_array(&mut buf, "gemma4.attention.head_count_kv", &[8, 8, 2, 8]);

        let mut cursor = Cursor::new(buf);
        let gguf = GgufFile::parse(&mut cursor).unwrap();

        assert_eq!(gguf.get_u32_array_mean("attention.head_count_kv"), Some(6));
        assert_eq!(gguf.get_u32_array_sum("attention.head_count_kv"), Some(26));
    }
}
