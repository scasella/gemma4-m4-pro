...
fn compute_tier_capacities(
    ...
    let gpu_runtime_overhead = config::gpu_runtime_overhead_bytes();
    let usable = total_ram.saturating_sub(memory_reserve);
...
fn estimate_kv_bytes(metadata: &ModelMetadata, context_length: u32) -> u64 {
    ...
    // 2 (key+value) * total_kv_heads * head_dim * 2 (FP16 bytes) * context
    2 * metadata.total_kv_heads() * head_dim * 2 * context_length as u64
}
...
fn try_sparse_moe_mmap(
    ...
    let fits_gpu = total_bytes <= caps.gpu_bytes + config::gpu_runtime_overhead_bytes();
    tracing::info!(
...
fn try_expert_streaming_assign(
    ...
    let gpu_budget_with_runtime = caps.gpu_bytes + config::gpu_runtime_overhead_bytes();
    if non_expert_bytes + estimated_pool > gpu_budget_with_runtime {
        tracing::debug!(
            ...
            gpu_budget_with_runtime as f64 / (1u64 << 30) as f64,
        );
        return None;
    }
...
fn try_dense_ffn_streaming_assign(
    ...
    let gpu_budget_with_runtime = caps.gpu_bytes + config::gpu_runtime_overhead_bytes();
    if non_ffn_bytes + estimated_pool > gpu_budget_with_runtime {
        tracing::debug!(
