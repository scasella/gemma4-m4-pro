...
impl Default for InferenceConfig {
    fn default() -> Self {
        Self {
            ...
            n_threads_batch: num_performance_cores(),
...
pub fn load_model(
    ...
    let headroom = config::keep_resident_headroom_bytes();
    let preload_headroom = config::preload_headroom_bytes();

    let keep_resident =
        nvme_bytes > 0 && (estimated_committed + nvme_bytes) <= total_ram.saturating_sub(headroom);

    let should_preload = keep_resident
        && (estimated_committed + nvme_bytes) <= total_ram.saturating_sub(preload_headroom);
...
pub fn compute_gpu_budget(
    ...
    // KV cache on GPU: 2 * total_kv_heads * head_dim * 2 bytes * context
    let head_dim = if metadata.num_heads > 0 {
        metadata.embedding_dim as u64 / metadata.num_heads as u64
    } else {
        0
    };
    let kv_on_gpu = 2 * metadata.total_kv_heads() * head_dim * 2 * context_length as u64;
    // Reserve room for Metal runtime plus a configurable memory cushion for the
    // rest of the machine. The research harness can raise this when it needs
    // Hypura to preserve more free memory for other apps.
    let runtime_overhead = config::gpu_runtime_overhead_bytes();
    let memory_reserve = config::memory_reserve_bytes();
    gpu_working_set
...
pub fn generate_with_nvme_scheduling(
    ...
    //
    // Keep-resident when: estimated_committed + nvme_bytes fits in RAM with the
    // configured headroom for page cache and the rest of the machine. This lets
    // small-spill models stay resident while large-spill models use streaming.
    let model_total_bytes = gguf.total_tensor_bytes();
    let buffer_bytes: u64 = gguf
        .tensors
        .iter()
    ...
    // (including all buffer layers pre-loaded) still fits under the configured
    // preload headroom.
    let should_preload = keep_resident
        && (estimated_committed + nvme_bytes) <= total_ram.saturating_sub(preload_headroom);
    ...
    // Only preload when the full model still fits under the configured preload
    // headroom.
    ...
    if !any_streaming && should_preload {
        ...
        tracing::info!(
            ...
            model_total_bytes as f64 / (1u64 << 30) as f64,
            total_ram.saturating_sub(preload_headroom) as f64 / (1u64 << 30) as f64,
        );
    }
