...
pub struct ChatTemplateMessage<'a> {
    pub role: &'a str,
    pub content: &'a str,
}
...
impl LlamaModel {
    ...
    ///
    /// Chat prompts contain model control tokens like `<bos>` and `<|turn>...`.
    /// Parse them as special tokens so the prompt matches llama.cpp CLI behavior.
...
/// Apply a chat template to plain role/content messages.
///
/// Returns `Ok(None)` when llama.cpp cannot recognize the template string.
pub fn apply_chat_template(
    template: Option<&str>,
    messages: &[ChatTemplateMessage<'_>],
) -> anyhow::Result<Option<String>> {
    let role_storage = messages
        .iter()
        .map(|message| CString::new(message.role))
        .collect::<Result<Vec<_>, _>>()?;
    let content_storage = messages
        .iter()
        .map(|message| CString::new(message.content))
        .collect::<Result<Vec<_>, _>>()?;

    let chat = role_storage
        .iter()
        .zip(content_storage.iter())
        .map(|(role, content)| hypura_sys::llama_chat_message {
            role: role.as_ptr(),
            content: content.as_ptr(),
        })
        .collect::<Vec<_>>();

    let template_storage = match template {
        Some(template) => Some(CString::new(template)?),
        None => None,
    };
    let template_ptr = template_storage
        .as_ref()
        .map_or(ptr::null(), |template| template.as_ptr());
    ...
    let mut written = unsafe {
        hypura_sys::llama_chat_apply_template(
            template_ptr,
            chat.as_ptr(),
            chat.len(),
            true,
    ...
    pub fn new(params: &SamplingParams) -> Self {
        ...
        unsafe {
            if params.temperature <= 0.0 {
                hypura_sys::llama_sampler_chain_add(ptr, hypura_sys::llama_sampler_init_greedy());
            } else {
