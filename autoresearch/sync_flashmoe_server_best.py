"""
Rebuild the exported stable Flash-MoE resident-server config from recorded runs.
...
def best_flashmoe_server_run(require_measured_runs: int = 3) -> tuple[dict[str, Any], Path] | None:
    ...
    for artifact_path in sorted(RUNS_DIR.glob("*.json"), reverse=True):
        ...
        if artifact.get("backend") != "flashmoe_server":
            continue
...
def main() -> int:
    best = best_flashmoe_server_run()
    if best is None:
        raise FileNotFoundError("No valid Flash-MoE resident-server run with at least three measured passes was found.")
    ...
    BEST_FLASHMOE_SERVER_CANDIDATE_PATH.write_text(
    ...
    record = {
        ...
        "candidate_export_path": str(BEST_FLASHMOE_SERVER_CANDIDATE_PATH),
        "summary": artifact["summary"],
        "measured_runs": int(candidate.get("measured_runs", 0)),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    BEST_FLASHMOE_SERVER_PATH.write_text(json.dumps(record, indent=2), encoding="utf-8")

    print(f"Best Flash-MoE server artifact:  {artifact_path}")
    print(f"Best Flash-MoE server record:    {BEST_FLASHMOE_SERVER_PATH}")
    print(f"Best Flash-MoE server candidate: {BEST_FLASHMOE_SERVER_CANDIDATE_PATH}")
